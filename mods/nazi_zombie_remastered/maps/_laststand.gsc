...
init()
{
	...
	precachestring( &"REMASTERED_ZOMBIE_BUTTON_TO_REVIVE_PLAYER" );
...
/*
		if( !self hasWeapon("ray_gun") && !self hasWeapon("sw_357") ) 
		{
			self.laststandpistol = weapon;
		}
*/

		// If player has Ray Gun, use ray gun, if not, check if they have 357, if yes then use it, otherwise we continue, and then say, if you still havent been assigned a pistol use default
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < players.size; i++ )
		{
			if ( players[i] can_revive( self ) )
			{
				...
				self.revivetrigger setHintString( &"REMASTERED_ZOMBIE_BUTTON_TO_REVIVE_PLAYER" );
				break;			
			}
		}
...
revive_do_revive( playerBeingRevived, reviverGun )
{
	...
	//CODER_MOD: TOMMYK 07/13/2008
	playerBeingRevived.revivetrigger setHintString( &"REMASTERED_ZOMBIE_BUTTON_TO_REVIVE_PLAYER" );
