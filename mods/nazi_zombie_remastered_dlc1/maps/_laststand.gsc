...
/*		if( solo && ( !self hasWeapon("ray_gun") || self GetAmmoCount("ray_gun") == 0  ) )
		...
		else if(!IsDefined( self.laststandpistol ) )
		{
			if(isDefined(self.last_stand_weapon))
			{
				self.laststandpistol = "zombie_colt"; // COOP normal pistol
			}
		}
		...
		case "fg42_bipod_deploying":
		case "mg42_bipod_deploying":
		case "30cal_bipod_deploying":
		case "bar_bipod_deploying":
		case "bipod_deploying":
