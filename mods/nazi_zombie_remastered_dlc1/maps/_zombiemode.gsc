...
main()
{
	...
	maps\_zombiemode_smoke_grenade::init();
...
track_ammo_count()
{
	...
	while ( true )
	{
		...
		if(!isDefined(weap) || weap == "none" || weap == "zombie_perk_bottle_doubletap" || weap == "zombie_perk_bottle_jugg" || weap == "zombie_perk_bottle_revive" || weap == "zombie_perk_bottle_sleight" || weap == "mine_bouncing_betty" || weap == "syrette" || weap == "m2_flamethrower_zombie" || weap == "m7_launcher_zombie" || weap == "zombie_melee" || weap == "falling_hands" )
		{
			continue;
		}
...
add_low_ammo_dialog()
{
	...
	{
		num_variants = maps\_zombiemode_spawner::get_number_variants(player_index + "nvox_ammo_low");
		self.vox_ammo_low = [];
		for(i=0;i<num_variants;i++)
		{
			self.vox_ammo_low[self.vox_ammo_low.size] = "nvox_ammo_low_" + i;	
		}
...
{
	sound = "plr_" + index + "_nvox_" + num +"play";
...
/*	level.zombieLeaderboardStatVariable["nazi_zombie_prototype"]["highestwave"] = "nz_prototype_highestwave"; // 3413
	...
	level.zombieLeaderboardStatVariable["nazi_zombie_prototype"]["totalpoints"] = "nz_prototype_totalpoints"; // 3415
	...
	*/
	level.zombieLeaderboardStatVariable["nazi_zombie_asylum"]["highestwave"] = "nz_asylum_highestwave"; // 3416
	...
	level.zombieLeaderboardStatVariable["nazi_zombie_asylum"]["totalpoints"] = "nz_asylum_totalpoints"; // 3418
	...
	/*
	level.zombieLeaderboardStatVariable["nazi_zombie_sumpf"]["highestwave"] = "nz_sumpf_highestwave"; // 3419
	...
	level.zombieLeaderboardStatVariable["nazi_zombie_sumpf"]["totalpoints"] = "nz_sumpf_totalpoints"; // 3421
	...
	level.zombieLeaderboardStatVariable["nazi_zombie_factory"]["highestwave"] = "nz_factory_highestwave"; // 3422
	...
	level.zombieLeaderboardStatVariable["nazi_zombie_factory"]["totalpoints"] = "nz_factory_totalpoints"; // 3424
...
init_fx()
{
	...
	level._effect["mg_placed"]	 	= loadfx( "impacts/large_woodhit_noshot" );
...
init_anims()
{
	...
	level.scr_anim["zombie"]["sprint1"] = %ai_zombie_sprint_v1;  // 1/3 early game (normal sprinters) (3/9)
	level.scr_anim["zombie"]["sprint2"] = %ai_zombie_sprint_v1;
	level.scr_anim["zombie"]["sprint3"] = %ai_zombie_sprint_v1;
	level.scr_anim["zombie"]["sprint4"] = %ai_zombie_sprint_v2;  // 1/3 early game (normal sprinters) (3/9)
	level.scr_anim["zombie"]["sprint5"] = %ai_zombie_sprint_v2;
	level.scr_anim["zombie"]["sprint6"] = %ai_zombie_sprint_v2;
	level.scr_anim["zombie"]["sprint7"] = %ai_zombie_sprint_v4; // 1/3 early game (super sprinters) (3/9)
	...
	level.scr_anim["zombie"]["crawl6"] 	= %ai_zombie_crawl_sprint; // Brought back the classic Nacht specific crawler anim
...
onPlayerConnect()
{
	for( ;; )
	{
		...
		player thread maps\_zombiemode_smoke_grenade::trackSmokeGrenade(); 
...
round_spawning()
{
	...
	if(level.round_number < 3 && get_players().size > 1 ) // if we're in co-op, for the first 2 rounds we spawn a bit more to account for players being split up
	{
		max = get_players().size * 3 + level.round_number;
	}
	else if ( level.first_round ) // for solo
	{
		max = int( max * 0.2 );	
	}
	else if (level.round_number < 3) // for solo
	{
		max = int( max * 0.4 );
	}
	else if (level.round_number < 4) // and we resume back on track with the same calcs for both solo and co-op here
	{
		max = int( max * 0.6 );
	}
...
play_intro_VO()
{
	...
	players[i] playsound ("plr_" + i + "_nvox_start" + "_" + randomintrange(0, variation_count));
...
round_think()
{
	...
	//setmusicstate( "WAVE_1" ); //CODE: UNSURE
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	if (sMeansOfDeath == "MOD_GRENADE_SPLASH" || sMeansOfDeath == "MOD_GRENADE")  // For all grenade explosive damage. Molotovs, M1 Launcher, Frags, and anything else
	{
		if( self.health > 75 )
		{
			...
			else if(isSubStr(sWeapon, "smoke") ) 
			{
				finalDamage = 10;
			}
	...
	if (sMeansOfDeath == "MOD_PROJECTILE_SPLASH" || sMeansOfDeath == "MOD_PROJECTILE")  // For all projectile explosive damage. Ray Gun, Panzer, Waffle, and anything else
	{
		if( self.health > 75 )
		{
			...
			else if(isSubStr(sWeapon, "ray_gun") ) 
			{
				...
				//finalDamage = radiusDamage(eInflictor.origin, 64,110,80, eAttacker, sMeansOfDeath );
			}
...
coop_player_spawn_placement()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		//players[i].respawn_point = structs[i]; CODE: UNSURE
...
nazizombies_set_new_zombie_stats()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		previous_play_time = players[i] zombieStatGet( "nz_asylum_timeinwave" );
		...
		players[i] zombieStatSet( "nz_asylum_timeinwave", int(level.current_play_time + previous_play_time) ); 

/*		if( getDvarInt( "classic_zombies") == 1 || getDvarInt( "super_sprinters") == 1 ) // if playing with 24 limit or with super sprinters disabled, these are considered cheats because they make the game easier. classic perks or grabby zombies is OK, these make the game harder
		{
			//iPrintLn("Highscores not saved, current Game Options configuration not allowed");
		}
		else
...
/*	if( level.systemLink || true == IsSplitScreen() )
	...
	{
		return self getStat( int(tableLookup( "mp/dlc1_achievements.csv", 1, dataName, 0 )) );
	}
	else
	{
...
/*	if( level.systemLink || true == IsSplitScreen() )
	...
	if(isDefined(override) && override == true)
	{
		self setStat( int(tableLookup( "mp/dlc1_achievements.csv", 1, dataName, 0 )), value );	
	}
	else
	{
