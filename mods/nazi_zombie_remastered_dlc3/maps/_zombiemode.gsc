...
round_spawn_failsafe()
{
	...
	while(1)
	{
		...
		if(self enemy_is_dog() && !flag("dog_round" ) && !isDefined(self.dog_recently_hit) && level.zombie_total < 10 ) // if almost near the end of a round we are extra careful and try to delete dogs that are not attacking the player, potentially stuck outside map?
		{
			wait ( 30 ); // wait another 30 just to be safe, at minimimum dog has to have not hit for 30 seconds, if not longer
			if(!isDefined(self.dog_recently_hit))
			{
				self dodamage( self.health + 100, (0,0,0) );
				break;
			}
		...
		self.dog_recently_hit = undefined;
