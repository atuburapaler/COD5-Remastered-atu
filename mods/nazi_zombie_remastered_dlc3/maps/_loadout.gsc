...
init_loadout()
{
	...
	level.startguns[level.startguns.size] = "zombie_tokarev";//change weapon, not needed for player randomizing
	level.startguns[level.startguns.size] = "zombie_nambu";//change weapon, not needed for player randomizing
	level.startguns[level.startguns.size] = "zombie_walther";//change weapon, not needed for player randomizing
...
init_models_and_variables_loadout()
{
	...
	else if( GetDvar( "zombiemode" ) == "1" || IsSubStr( level.script, "nazi_zombie_" ) || level.script == "zombie_test_map" ) // CODER_MOD (Austin 5/4/08): zombiemode loadout setup
	{
		...
		add_weapon( "zombie_tokarev" );
		add_weapon( "zombie_nambu" );
		add_weapon( "zombie_walther" );
		PrecacheModel( "viewmodel_hands_cloth_marine_bare" );
		//PrecacheModel( "viewmodel_rus_spy_arms" );
		//PrecacheModel( "viewmodel_jap_officer_arms" );
		//PrecacheModel( "viewmodel_ger_scientist_arms" );
...
give_model( class )
{
	...
	else if (level.campaign == "american")
	{
		...
		else if( isDefined( level.use_zombie_heroes ) && level.use_zombie_heroes &&	players.size == 1 ) 
		{
